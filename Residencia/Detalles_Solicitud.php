...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (isset($_POST['Estatus'], $_POST['Comentarios'])) {
            ...
            if ($consulta && mysqli_num_rows($consulta) > 0) {
                ...
                // Actualizar el estatus y comentarios en la base de datos
                $sql = "UPDATE solicitudes SET Estatus = '$Estatus', Id = '$IdAdmin', ComentariosAdmin = '$Comentarios', Lote = $Id_Lote WHERE Id_Solicitud = $Id_Solicitud";
                $resultado = mysqli_query($conexion, $sql);
        
                // Si se subió un archivo, se valida
                if ($_FILES['Archivo']['error'] == UPLOAD_ERR_OK) {
                    $Archivo = $_FILES['Archivo']['tmp_name'];
                    $Nombre_Archivo = $_FILES['Archivo']['name'];
                    $Destino = "ArchivosPdf/";

                    move_uploaded_file($Archivo, $Destino.$Nombre_Archivo);
        
                    $Ruta = "UPDATE solicitudes SET Respuesta = 'ArchivosPdf/$Nombre_Archivo' WHERE Id_Solicitud = $Id_Solicitud";
                    mysqli_query($conexion, $Ruta);
                }
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'error, no hay un lote activo',
                })
                </script>
                <?php        
            }   
        ...
        if(mysqli_num_rows($Consulta) > 0) {
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <h5>Estatus: </h5>
                        <select name="Estatus">
                            <option value="Pendiente" <?= ($fila['Estatus'] == 'Pendiente') ? 'selected' : ''; ?>>Pendiente</option>
                            <option value="Aprobado" <?= ($fila['Estatus'] == 'Aprobado') ? 'selected' : ''; ?>>Aprobado</option>
                            <option value="Rechazado" <?= ($fila['Estatus'] == 'Rechazado') ? 'selected' : ''; ?>>Rechazado</option>
                        </select>
                    </div>                
                </div>
                <div class="col-md-6">
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <label id="Contador">0/255</label>
                    <textarea maxlength="255" id="Comentarios" name="Comentarios"></textarea>
                    </div>         
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label>Ingresar documento en pdf con la respuesta al estudiante: </label><br>
                    ...
                    <table class="table table-bordered" id="dataTable" width="100%">
                        ...
                        <?php
                            ...
                            while ($filaDetallesSolicitud = mysqli_fetch_assoc($ConsultaDetallesSolicitudes)) {
                                ...
                                ?>
                                <tr>
                                    <?php while ($fila2 = mysqli_fetch_assoc($sql2)): ?>
                                    ...
                                    <td><?php echo $filaDetallesSolicitud['Comentarios']; ?></td><?php
